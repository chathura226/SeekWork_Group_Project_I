...
<div class="pagetitle column-12">
    ...
    <nav>
        ...
        <ul class="breadcrumbs">
            ...
            <li class="breadcrumbs__item">
                <a href="<?= ROOT ?>/moderator/tasks" class="breadcrumbs__link">Tasks</a>
            </li>
            <li class="breadcrumbs__item">
...
<div class="task-details-wrapper column-12">
    ...
    <div class="btn-container c-s-8 c-e-13">
        <div class="btn-effect">
            ...
            <h1>Rs.<?= $task->value ?>/=</h1>
            <?php if ($task->status == 'active' || $task->status == 'disabled'): ?>
                <?php if ($task->status == 'disabled'): ?>
                    <a class="effect popouplink" data-id="<?= $task->taskID ?>" data-action="enable"
                       title="Enable Task">Enable Task</a>
                <?php else: ?>
                    <a class="effect popouplink" data-id="<?= $task->taskID ?>" data-action="disable"
                       title="Disable Task">Disable Task</a>
                <?php endif;endif; ?>
    ...
    <?php if (!empty($student)) : ?>
        ...
        $student->assigned = '1';
        $this->view('includes/studentDetail', (array)$student) ?>
    ...
    <div class="popup-content" style="background-color: transparent;border: none;">
        <div class="wrapper">
            <div class="form-wrap column-12 row-6">
                <div class="tab-form row-" style="padding-left: 10px;">
                    <div class="myheader">
                        <div class="active-login"><h2></h2></div>
                    ...
                    <div class="tab-body ">
                        <div class="active1">
                            <form method="post">
                                ...
                                <input value="" name="taskID" id="taskID" hidden>
                                <input value="" name="action" id="action" hidden>

                                <div class="form-input">
                                    <label>Reason</label>
                                    <textarea rows="10" cols="35" id="enableDisableReason" name="enableDisableReason"
...
<script>
    ...
    var verificationIDInput = document.getElementById('taskID');
    ...
    linkArray.forEach(function (item) {
        item.addEventListener('click', function (event) {
            ...
            actionInput.value = item.getAttribute('data-action')
            document.querySelector('.active-login h2').innerHTML = item.textContent;
