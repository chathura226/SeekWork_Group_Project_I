...
if(isset($_GET['page_id'])){
    ...
    if($limit){
        ...
        reviews_per_pagination_page'], POD:: PARAM_INT);
        
        $stmt->bindValue(':review_per_pagination_page', (int)$_GET['review_per_pagination_page'], POD::PARAM_INT);

    }

    $stmt->bindValue(':page_id', (int)$_GET['page_id'], POD::PARAM_INT);
    $stmt->execute();
    $reviews = $stmt->fetchAll(POD::FETCH_ASSOC);

    $stmt = $pod->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM reviews WHERE
    page_id=?');

    $stmt->execute([$_GET['page_id']]);
    $reviews_info = $stmt->fetch(POD::FETCH_ASSOC);
...
<div class="con">
    ...
    <select class="sort_by" id="sort_by">
        ...
        <option value="rating_lowest"<?=isset($_GET['sort_by']) && $_GET['sort_by'] == 'oldest' ? 'selected' : ' '?>>Oldest</option>   
    </select>
